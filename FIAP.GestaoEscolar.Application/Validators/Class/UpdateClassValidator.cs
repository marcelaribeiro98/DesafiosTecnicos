...
namespace FIAP.GestaoEscolar.Application.Validators.Class
{
    public class UpdateClassValidator : AbstractValidator<UpdateClassRequest>
    {
        ...
        public UpdateClassValidator(IClassService classService)
        {
            _classService = classService;

            RuleFor(x => x.Id)
            .NotEmpty().WithMessage("O id da turma é obrigatório.")
            .GreaterThan(0).WithMessage("O id da turma deve ser um número positivo.");
        ...
        private async Task<bool> BeUniqueClassName(UpdateClassRequest request, string className, CancellationToken cancellationToken = default)
        {
